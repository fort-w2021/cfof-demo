...
# Step 1
cfof <- function(data, ratio = .1) {
  # check data, ratio
  # get neighborhood sizes for each datum for each k
  # for each datum, find minimal k* so that it is in at least ratio * nrow(data) k*-neighborhoods
...
cfof <- function(data, ratio = .1, distance = "euclidean") {
  # check data, ratio
  neighborhood_sizes <- get_neighborhood_sizes(data, distance)
...
# Step 3:
# figure out computation of neighborhood sizes,
...
get_neighborhood_sizes <- function(data, distance) {
  distance_matrix <- distance_function(data, distance)
  ...
  distance_ranks <- apply(distance_matrix, MAR = 1, FUN = rank)
  n <- nrow(data)
