...
#'
#' The concentration-free outlier factor is the smallest neighborhood size k
#' in terms of "k nearest neighbors" for which the object is a neighbor of at
#' least a certain ratio of other objects.
#' "Loosely speaking, it corresponds to a measure of how many nearest neighbors
#' have to be taken into account in order for a point to be close to a
#' sufficient fraction of the data." (Angiulli, 2019)
#' See reference below for details.
#'
#' @param data a data.frame or matrix. rows are instances, columns are features.
#'   factor variables are turned into dummy variables and then treated as numeric features.
...
#' @param distance what type of distance to compute. defaults to euclidean, see ?dist
...
#'   other objects". Rows are data instances, columns correspond to different ratios.
#' @references
#'   Angiulli, F. (2019)
...
cfof <- function(data,
  ...
  distance_matrix <- as.matrix(dist(data_prepped, method = distance))
...
#' @return n x n matrix with entries i,j: "how many neighborhoods of size j is object i in"
count_neighborhoods <- function(distance_matrix) {
  ...
  # ties = "max", "average" would result in systematically too small neighborhoods (ranks too high)
  # ties = "min" in systematically too large neighborhoods (ranks too low)
  # ties = "first" would yield smaller neighborhoods for objects higher up in data set.
